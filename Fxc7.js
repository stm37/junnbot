...
memberLimit = 3
OwnerNumber = 62895605030379
instagram = instagram.com/junaediiii7_/
limitt = 20
botinfo = setting.botinfo
prefix = !
name = JUNNBOT
yt = https://www.youtube.com/channel/UCjJrwVftfya5cg9uY2Fw-Rg
ban = []
...
function addMetadata(packname, author) {
				if (!packname) packname = 'termux-bot-wa'; if (!author) author = ' junaedi';
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} JUNN BOT MISS YOU:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			only: {
				...
				userB: `Hai ${pushname2} Kamu Belom Terdaftar Didatabase Silahkan Ketik \n${prefix}daftar`,
			...
			const ownerNumber = [`$62895605030379@s.whatsapp.net`] // owner number ubah aja 
			const ownerInfo = `${62895605030379}`
			...
			const FarhanGans = ["0895605030379@s.whatsapp.net"]
			const FarhanGans2 = " ~ ᴊᴜɴɴ ʙᴏᴛ"
			const isUrl = (url) => {
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		break
			...
			case 'brainly':
					...
					costum(teks, text, junaediindra, `Ciee Cari Jawaban Yaa😂\nFollow IG: @_junaediiii7_`)
					console.log(res)
					})
					await limitAdd(sender)
				break 
		...
		case 'verify':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen JUNN BOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				user.push(sender)
				costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal(), groupName, premi, Simihh, Welcomee, ModeAnime, Nsfww, BadWordd, AntiLinkk, AntiVirtexx), text, junaedii, junaediii2)
			break 
...
//akhir kreator
		    ...
		    try {
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat JUNNBOT\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *JUNNBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
...
// akhir encrypt & decrypt Fitur
	...
	// new features
		...
		case 'premiumlist':
			...
			nomorList++
			listPremi += `${62895605030379. wa.me/${getAllPremiumUser()[i].split("@")[0]}\n➸ *Expired*: ${checkExp.days} day(s) ${checkExp.hours} hour(s) ${checkExp.minutes} minute(s)\n\n`
			}
		...
		break 
				default:
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[JUNNBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
			}
		} catch (e) {
